...
bool isSorted(const vector<int> &vec) {
    ...
    atomic<bool> arraySorted = true;

    auto func = [&vec, &arraySorted](uint from, uint to) -> bool {
        bool partSorted = true;
        for (uint i = from + 1; i < to && partSorted && arraySorted; ++i) {
            partSorted = vec[i] >= vec[i - 1];
        }
        if (!partSorted) {
            arraySorted = false;
        }
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        future<bool> future = async(launch::async, func, starts[i], starts[i + 1]);
